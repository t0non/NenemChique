...
export function DataProvider({ children }: { children: React.ReactNode }) {
  ...
  const mapSettingsFromDB = (s: any): SiteSettings => ({
    ...
    showInstallments: s.show_installments ?? true,
    installmentCount: s.installment_count ?? 12,
    cashDiscountPercent: s.cash_discount_percent ?? 5,
  });
  ...
  const mapSettingsToDB = (s: SiteSettings) => ({
    ...
    show_installments: s.showInstallments ?? true,
    installment_count: s.installmentCount ?? 12,
    cash_discount_percent: s.cashDiscountPercent ?? 5,
  });
    ...
    try {
      ...
      const fetchPromise = (async () => {
        ...
        try {
          ...
          if (!sError && sData) {
            let mappedSettings = mapSettingsFromDB(sData);
            // Merge localStorage installment settings if DB columns don't exist yet
            try {
              const localInstallRaw = localStorage.getItem('nenem_installment_settings');
              if (localInstallRaw) {
                const localInstall = JSON.parse(localInstallRaw);
                if (sData.show_installments === undefined || sData.show_installments === null) {
                  mappedSettings = { ...mappedSettings, ...localInstall };
                }
              }
            } catch {}
  ...
  const updateSettings = async (s: SiteSettings) => {
    const supabase = await loadSupabase();
    const fullPayload = { id: 1, ...mapSettingsToDB(s) };
    let { error } = await supabase.from('settings').upsert([fullPayload]);
    if (error) {
      // Fallback: try without the new columns in case they don't exist in DB yet
      const { show_installments, installment_count, cash_discount_percent, show_coupon_cta, ...safePayload } = fullPayload;
      const { error: fallbackError } = await supabase.from('settings').upsert([{ id: 1, ...safePayload }]);
      if (fallbackError) throw fallbackError;
    }
    // Always persist installment settings to localStorage as reliable client-side source
    try {
      localStorage.setItem('nenem_installment_settings', JSON.stringify({
        showInstallments: s.showInstallments ?? true,
        installmentCount: s.installmentCount ?? 12,
        cashDiscountPercent: s.cashDiscountPercent ?? 5,
        showCouponCTA: s.showCouponCTA ?? true,
      }));
    } catch {}
    setSettings(s);
    await fetchData();
