...
export function CartSheet() {
  ...
  const finalizeOrder = async () => {
    const telefoneLoja = "5531996244487";
    ...
    // Disparo manual para o GTM (Google Tag Manager) para garantir o registro do clique antes do redirecionamento
    if (typeof window !== 'undefined') {
      window.dataLayer = window.dataLayer || [];
      window.dataLayer.push({ 'event': 'click_whatsapp_checkout' });
    }
            ...
            <div className="space-y-3">
                ...
                <div key={item.id} className="relative grid grid-cols-[64px_1fr_auto] gap-3 bg-white p-3 md:p-3.5 rounded-2xl border border-muted/10 items-center">
                  <div className="relative w-16 h-16 rounded-xl overflow-hidden shrink-0 bg-muted/10">
                    <Image src={(item.images && item.images[0]) || 'https://picsum.photos/seed/baby/200/200'} alt={item.name} fill className="object-contain" sizes="64px" loading="lazy" />
                  </div>
                      ...
                      <div className="flex flex-wrap gap-1 mt-1">
                        {(item as any).selectedSize && (
                          <span className="text-[9px] font-black uppercase tracking-wider bg-primary/10 text-primary px-1.5 py-0.5 rounded-md">
                            Tam: {(item as any).selectedSize}
                          </span>
                        )}
                        {(item as any).selectedColor && (
                          <span className="text-[9px] font-bold uppercase tracking-wider bg-secondary/10 text-secondary-foreground px-1.5 py-0.5 rounded-md">
                            Cor: {(item as any).selectedColor}
                          </span>
                        )}
                      </div>
                    )}
                    <p className="text-xs md:text-sm text-foreground font-bold mt-1">R$ {item.price.toFixed(2).replace('.', ',')}</p>
                  </div>
        ...
        <div className="p-3 md:p-4 bg-white border-t flex flex-col gap-3 shrink-0 pb-[env(safe-area-inset-bottom)]">
          ...
          <div className="space-y-2">
            <Button 
              ...
              className="w-full h-11 rounded-full text-sm font-bold bg-emerald-500 hover:bg-emerald-600 text-white border-none gap-2 track-btn-whatsapp"
              data-gtm="whatsapp"
            >
